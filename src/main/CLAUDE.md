...
- **Master toggle** (`settings.cron.enabled`, global machine-local, opt-in) gates FIRING only — watching/validation/UI stay live when off. cron.ts is injected via `initCron({ readSettings, writeSettings, runAgentTurn, getWindow })` (no import of main). IPC: `cron:read` / `cron:setEnabled` / `cron:setJobEnabled` / `cron:runNow` / `cron:setMaxCatchupHours` / `cron:setMaxRunMinutes`; push events `cron:state` + `cron:chatsChanged`.
