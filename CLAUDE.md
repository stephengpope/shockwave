...
- `settings.ts` — typed `Settings` shape. (`DEFAULT_SETTINGS` lives in `src/main/settingsStore.ts`, next to the store that applies it.)
...
- The `Settings` type lives in `src/shared/settings.ts`; `DEFAULT_SETTINGS` lives in `src/main/settingsStore.ts`. Top-level keys: `workspaces`, `activeWorkspaceId` (both DERIVED from the workspace tables, not stored scalars), `appearance`, `codingAgent`, `agentSecrets[]`, `transcription`, `sync`, `cron`, `chatSidebarOpen`, `chatSidebarWidth`, `sidebarWidth`, `viewMode`, `treeSortOrder`, `bookmarkFilterActive`, `windowBounds`. (`cron` = scheduled-runs master toggle + windows, machine-local; job definitions live per-workspace in `<workspace>/cron.json`. See "Scheduled runs (cron)" in `src/main/CLAUDE.md`.) Adding a persisted field means updating the `Settings` type + `DEFAULT_SETTINGS`, and the corresponding state slice in `useSettings` (which holds the canonical in-memory copy and is the only place that calls `persistSettings()`). **There is no deep-merge to extend** — scalar settings are stored one row per leaf key, so a key with no row simply falls back to `DEFAULT_SETTINGS`. If the new field holds a credential, add its key pattern to `SETTINGS_SECRET_PATTERNS` in `src/main/settingsKeys.js` — that list routes it to the `secret_value` table, whose crypto columns are `NOT NULL`. Persistence spans five tables (`setting`, `workspace`, `workspace_local`, `agent_secret`, `secret_value`) but `readSettings`/`writeSettings` hide that entirely; see "Settings persistence + secrets encryption" in `src/main/CLAUDE.md`.
